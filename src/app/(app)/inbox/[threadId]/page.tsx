...
import { getThread } from "@/queries/email";
...
export default async function ThreadPage({ params }: { params: Promise<{ threadId: string }> }) {
  ...
  const [thread, users] = await Promise.all([getThread(threadId), listActiveUsers()]);
  if (!thread) notFound();
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <Link href="/inbox" className="text-xs text-gray-500 hover:text-brand-700">
            ← Inbox
          </Link>
